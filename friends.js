...
(function () {
  ...
  function closeSharedView(){var target=document.getElementById("sharedView"),home=document.getElementById("sharingContainer");if(target){target.hidden=true;target.replaceChildren();}if(home)home.hidden=false;}
  function sharedBackButton(){return button("← Retour à Partage et amis",closeSharedView,"sharing-view-back");}
  function renderSharedTable(container,data,friend){container.replaceChildren();container.hidden=false;container.append(sharedBackButton(),el("span","eyebrow","Consultation seule"),el("h3","","Tableur de "+friend.displayName),el("p","","Seules les colonnes et les lignes autorisées sont affichées. Aucune modification n’est possible."));var sheets=data&&data.sheets||[];if(!sheets.length){container.append(el("div","sharing-empty","Aucune donnée de tableur n’est disponible pour cet accès."));return;}sheets.forEach(function(sheet){var wrap=el("div","shared-table-wrap"),table=el("table","shared-table"),head=document.createElement("thead"),headRow=document.createElement("tr");headRow.append(el("th","","Ligne"));for(var c=0;c<sheet.cols;c++)headRow.append(el("th","",sheet.headers[c]||MONTHS[c]));head.append(headRow);table.append(head);var body=document.createElement("tbody");for(var row=0;row<sheet.rows;row++){var line=document.createElement("tr");line.append(el("th","",sheet.rowHeaders[row]||("Ligne "+(row+1))));for(var col=0;col<sheet.cols;col++){var cell=sheet.cells&&sheet.cells[row+","+col];line.append(el("td","",cell&&cell.raw!==undefined?String(cell.raw):"—"));}body.append(line);}table.append(body);wrap.append(table);container.append(el("h4","shared-sheet-title",sheet.name),wrap);});}
  function renderSharedDashboard(container,data,friend){container.replaceChildren();container.hidden=false;container.append(sharedBackButton(),el("span","eyebrow","Consultation seule"),el("h3","","Dashboard de "+friend.displayName),el("p","","Synthèse calculée uniquement avec les colonnes et lignes que ton ami a autorisées."));var months=data&&data.months||[];if(!months.length){container.append(el("div","sharing-empty","Aucune donnée de dashboard n’est disponible pour cet accès."));return;}var income=months.reduce(function(sum,item){return sum+Number(item.salary||0);},0),expenses=months.reduce(function(sum,item){return sum+Math.abs(Number(item.expenses||0));},0),savings=months.reduce(function(sum,item){return sum+Math.abs(Number(item.savingsTotal||0));},0),grid=el("div","chart-detail-grid");[["Mois analysés",String(months.length)],["Revenus",money(income)],["Dépenses",money(expenses)],["Épargne",money(savings)]].forEach(function(metric){var card=el("div"),title=el("span","",metric[0]),value=el("strong","",metric[1]);card.append(title,value);grid.append(card);});container.append(grid);var list=el("div","shared-month-list");months.forEach(function(month){var line=el("div","shared-month-row"),title=el("strong","",month.label),total=el("span","",money(Number(month.salary||0)-Math.abs(Number(month.expenses||0))));line.append(title,total);list.append(line);});container.append(list);}
  async function viewShared(friend,kind){var target=document.getElementById("sharedView"),home=document.getElementById("sharingContainer");if(!target)return;if(home)home.hidden=true;target.hidden=false;target.replaceChildren(el("p","sharing-view-loading","Ouverture de l’accès partagé…"));target.tabIndex=-1;target.focus({preventScroll:true});target.scrollIntoView({behavior:"smooth",block:"start"});var result=await loadShared(friend.friendId,kind);if(result.error){target.replaceChildren(sharedBackButton(),notice(result.error,"error"));return;}if(kind==="dashboard")renderSharedDashboard(target,result.payload,friend);else renderSharedTable(target,result.payload,friend);target.focus({preventScroll:true});}
  function person(friend,subtitle,actions){var row=el("article","sharing-person"),identity=el("div","sharing-person-identity"),avatar=el("span","sharing-person-avatar",String(friend.displayName||"A").trim().slice(0,1).toLocaleUpperCase("fr")),copy=el("div","sharing-person-copy"),buttons=el("div","sharing-actions");copy.append(el("h4","",friend.displayName),el("p","",subtitle));identity.append(avatar,copy);actions.forEach(function(action){buttons.append(action);});row.append(identity,buttons);return row;}

...
(function () {
  ...
  async function removeFriend(id){
    ...
    // Le serveur retire la relation ET les accès dans les deux sens, atomiquement.
    // Aucun repli vers plusieurs suppressions navigateur : une erreur laisse tout intact.
    try{var result=await client.rpc("remove_friendship",{p_friendship_id:id});if(result.error)return{error:errorText(result.error)};invalidate();return{success:true};}catch(e){return{error:errorText(e)};}
  }
  ...
  async function renderSharing(container,options){
    ...
    var shell=el("div","sf-shell"),toolbar=el("div","sf-toolbar"),tabs=el("div","sf-tabs"),content=el("div","sf-tab-content"),refresh=action("Actualiser",function(){window.GFUI.withBusy(refresh,function(){return renderSharing(container,{force:true,animated:true});});},"refresh","refresh");tabs.setAttribute("role","tablist");tabs.setAttribute("aria-label","Amis et invitations");
